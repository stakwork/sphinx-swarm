...
fn vls_ports(idx: u16) -> (String, String, String, String) {
    let rpc_port = 9375 + idx;
    let grpc_port = 10019 + idx;
    let mqtt_port = 1883 + idx;
    let rocket_port = 8000 + idx;
    (
        rpc_port.to_string(),
        grpc_port.to_string(),
        mqtt_port.to_string(),
        rocket_port.to_string(),
    )
}

pub fn cln_vls(name: &str, idx: u16, links: Vec<&str>) -> Config<String> {
    let ps = vls_ports(idx);
    let ports = vec![ps.0.as_str(), ps.1.as_str(), ps.2.as_str(), ps.3.as_str()];
    let vols = vec!["/root/.lightning"];
    Config {
        image: Some("sphinxlightning/sphinx-cln-vls:0.1.2".to_string()),
        hostname: Some(name.to_string()),
        cmd: Some(vec![
            format!("--alias=sphinx-{}", name),
            format!("--addr=0.0.0.0:{}", &ps.0),
            format!("--grpc-port={}", ps.1),

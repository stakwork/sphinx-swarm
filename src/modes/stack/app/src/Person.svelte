<script>
    ...
    const signedUp = owner_alias ? true : false;
    ...
    {#if selected}
      <div class="fields">
        ...
        <section class="value-wrap">
          ...
          <span on:click={copyToClipboard(owner_pubkey)}><CopyIcon size={0} class="copy-icon" /></span>
        </section>
        {#if owner_route_hint}
          ...
          <section class="value-wrap">
            ...
            <!-- svelte-ignore a11y-click-events-have-key-events -->
            <span on:click={copyToClipboard(owner_route_hint)}><CopyIcon size={0} class="copy-icon" /></span>
          </section>
        {/if}
        <p class="user-values-title">Invite QR code</p>
        <QrCode padding={1.5} value={owner_pubkey} />
      </div>
    {:else}
